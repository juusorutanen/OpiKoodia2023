    ...
    return async (dispatch) => {
        ...
        if(response.ok) {
            ...
            if(response.status === 409) {
                ...
                dispatch(registerFailed("Register failed. Server responded with a satus "+response.status+" "+response.statusText));
            }
        }
    }
}
...
export const logout = (token) => {
    return async (dispatch) => {
        ...
        dispatch(stopLoading());
        if(!response) {
            dispatch(logoutFailed("Server responded. Logging you out."));
            return;
        }
        if(response.ok) {
